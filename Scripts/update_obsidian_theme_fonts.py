...
def write_plugin(faces: list[dict]) -> None:
    ...
    # Live Preview post-process wraps FE00 / half-slice runs in a span with
    # the matching `edenia … h` family when that face is enabled.
...
function splitPinned(text, settings) {{
  ...
  while (i < cps.length) {{
    ...
    const family = hasHalfSliceMark(run) ? familyForCps(run, settings) : null;
    parts.push({{
