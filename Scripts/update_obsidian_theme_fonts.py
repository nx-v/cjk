...
def write_plugin(faces: list[dict]) -> None:
    ...
    manifest = {
        ...
        "description": (
            ...
            "and pins digraph runs to the matching segment face "
    ...
    # Live Preview post-process wraps FE00 / slice-VS runs in a span with
    # the single matching family (qh / qv / q / t / h).
...
function isEdeniaCp(cp) {{
  return (
    (cp >= 0xa000 && cp <= 0xa4c6) ||
    (cp >= 0xe000 && cp <= 0xf8ff) ||
    (cp >= 0xf0000 && cp <= 0xf1fff) ||
    (cp >= 0xfe00 && cp <= 0xfe0f) ||
    (cp >= 0xe0100 && cp <= 0xe011f) ||
    cp === 0x034f ||
    (cp >= 0x0300 && cp <= 0x036f) ||
    (cp >= 0x1ab0 && cp <= 0x1aff) ||
    (cp >= 0x1dc0 && cp <= 0x1dff) ||
...
function familyForCps(cps) {{
  let face = null;
  if (cps.some((c) => c >= 0xe0118 && c <= 0xe011f)) face = "q";
  else if (cps.some((c) => c >= 0xe0111 && c <= 0xe0117)) face = "qh";
  else if (cps.some((c) => c >= 0xe010a && c <= 0xe0110)) face = "qv";
  else if (cps.some((c) => c >= 0xe0100 && c <= 0xe0109)) face = "t";
  else if (cps.some((c) => c >= 0xfe08 && c <= 0xfe0f)) face = "h";
  else if (cps.includes(0xfe00)) face = "h";
  else return null;
  ...
  if (yi) return face ? "edenia yi " + face : null;
  if (kana) return face ? "edenia kana " + face : null;
  return null;
}}

function splitPinned(text) {{
  ...
  while (i < cps.length) {{
    if (!isEdeniaCp(cps[i])) {{
      let j = i + 1;
      while (j < cps.length && !isEdeniaCp(cps[j])) j++;
      parts.push({{
        text: String.fromCodePoint(...cps.slice(i, j)),
        family: null,
      }});
      i = j;
      continue;
    ...
    const family = hasSliceMark(run) ? familyForCps(run) : null;
    parts.push({{
...
function pinEdeniaDigraphs(root) {{
  ...
  for (const textNode of nodes) {{
    ...
    if (!raw || !hasSliceMark([...raw].map((ch) => ch.codePointAt(0)))) continue;
    const parts = splitPinned(raw);
    ...
    for (const part of parts) {{
      if (part.family) {{
        ...
        span.style.fontFamily = "'" + part.family + "'";
...
module.exports = class {PLUGIN_CLASS} extends Plugin {{
  async onload() {{
    ...
    this.registerMarkdownPostProcessor((el) => pinEdeniaDigraphs(el));
  }}
}};
"""
