...
class LegacySerifStrokeDrawer:
    ...
    def __draw_curve_universal(
        ...
        if delta1 != 0:
            vec_d = (
                Vec2(0, delta1)
                if all(vec_1 == vec_s1)
                else normalize(vec_1 - vec_s1, delta1)
            )
            vec_1 += vec_d
        ...
        if delta2 != 0:
            vec_d = (
                Vec2(0, -delta2)
                if all(vec_2 == vec_s2)
                else normalize(vec_2 - vec_s2, delta2)
            )
            vec_2 += vec_d

        self.__draw_curve_body(
            vec_1, vec_s1, vec_s2, vec_2, a1, a2, kMinWidthT, opt3, opt4
        )
        self.__draw_curve_head(
            vec_1, vec_s1, a1, kMinWidthT, vec_1.y <= vec_2.y, corner_offset
        )  # XXX: should check NaN or inf?
        self.__draw_curve_tail(
            vec_s2, vec_2, a1, a2, kMinWidthT, hane_adjustment, opt4, vec_2.y <= vec_1.y
        )

    def __draw_curve_body(
        self,
        vec_1: Vec2,
        vec_s1: Vec2,
        vec_s2: Vec2,
        ...
        def delta_d(t: float) -> float:
            ...
            if is_quadratic and (opt3 > 0 or opt4 > 0):
                return (
                    (self.font.kMinWidthT - opt3 / 2) - (opt4 - opt3) / 2 * t
                ) / self.font.kMinWidthT
            else:
                return 1
        ...
        if (
            a1 == 132 or a1 == 22 and (vec_1.y > vec_2.y)
            if is_quadratic
            else (vec_1.x > vec_s1.x)
        ):
            for index in range(len(right) - 1):
                ...
                if point1.y <= vec_1.y and vec_1.y <= point2.y:
                    ...
                    new2 = Vec2(
                        ...
                        vec_1.y if a1 == 132 else vec_1.y + 1,
                    )
        ...
        path = svgwrite.path.Path(
            d="M" + (" L".join(dots)), stroke="black", stroke_width=0, fill="black"
        )
        self.canvas.add(path)

    def __draw_curve_head(
        self,
        vec_1: Vec2,
        vec_s1: Vec2,
        a1: int,
        ...
        if a1 == 12:
            degree = (
                np.arctan2(vec_1.x - vec_s1.x, vec_s1.y - vec_1.y) / (np.pi * 2) * 360
            )
            polygon = [
                Vec2(-kMinWidthT, 0),
                Vec2(+kMinWidthT, 0),
            ...
            if is_up_to_bottom:
                # from up to bottom
                degree = (
                    np.arctan2(vec_1.x - vec_s1.x, vec_s1.y - vec_1.y)
                    / (np.pi * 2)
                    * 360
                )
                head_type = (
                    np.arctan2(np.abs(vec_1.y - vec_s1.y), np.abs(vec_1.x - vec_s1.x))
                    / np.pi
                    * 2
                    - 0.4
                )
                ...
                # bottom to up
                degree = (
                    np.arctan2(vec_s1.y - vec_1.y, vec_s1.x - vec_1.x)
                    / (np.pi * 2)
                    * 360
                )
                polygon = [
            ...
            polygon += (
                [Vec2(0, +kMinWidthT + 2), Vec2(0, 0)]
                if a1 == 27
                else [Vec2(-kMinWidthT, +kMinWidthT + 4)]
            )
            # draw
            polygon = [str(i) for i in polygon]
            path = svgwrite.path.Path(
    ...
    def __draw_curve_tail(
        ...
        if a2 in [1, 8, 15]:
            degree = (
                np.arctan2(vec_2.y - vec_s2.y, vec_2.x - vec_s2.x) / (np.pi * 2) * 360
            )
            ...
            tail_type = (
                np.arctan2(np.abs(vec_2.y - vec_s2.y), np.abs(vec_2.x - vec_s2.x))
                / np.pi
                * 2
                - 0.6
            )
            ...
            polygon = [
                ...
                Vec2(0, -kMinWidthT * self.font.kL2RDfatten),
                Vec2(
                    np.abs(tail_type) * kMinWidthT * self.font.kL2RDfatten,
                    pm * kMinWidthT * self.font.kL2RDfatten,
                ),
            ]
            polygon = [str(i) for i in polygon]
            path = svgwrite.path.Path(
            ...
            hane_length = (
                ...
                * 4
                * np.min(
                    [1 - hane_adjustment / 10, (kMinWidthT / self.font.kMinWidthT) ** 3]
                )
                * jump_factor
            )
            polygon = [
    ...
    def draw_bezier(
        ...
        self.__draw_curve_universal(
            vec_1, vec_s1, vec_s2, vec_2, a1, a2, opt1, hane_adjustment, opt3, opt4
        )
    ...
    def draw_curve(
        ...
        self.__draw_curve_universal(
            vec_1, vec_s, vec_s, vec_2, a1, a2, opt1, hane_adjustment, opt3, opt4
        )
    ...
    def draw_line(
        ...
        a2: int,
        opt1,
        uroko_adjustment: int,
        kakato_adjustment: int,
    ):
        kMinWidthT = self.font.kMinWidthT - opt1 / 2
        # Full encoded codes (e.g. 3032) are passed in; match styles on % 100.
        a1 = int(a1) % 100
        a2 = int(a2) % 100
        ...
        if (
            ...
            and (
                vec_1.x > vec_2.x
                or np.abs(vec_2.y - vec_1.y) >= np.abs(vec_2.x - vec_1.x)
                or a1 == 6
                or a2 == 6
            )
        ):
            ...
            cosrad, sinrad = (
                Vec2(0, 1) if (vec_1.x == vec_2.x) else normalize(vec_2 - vec_1)
            )

            rotation_matrix = np.array([[sinrad, cosrad], [-cosrad, sinrad]])

            # Distinct Vec2s — ``[v] * 4`` aliases one object and corrupts tips.
            poly0 = [Vec2(0, 0), Vec2(0, 0), Vec2(0, 0), Vec2(0, 0)]

            # head
            if a1 == 0:
                poly0[0] = rotation_matrix @ Vec2(kMinWidthT, self.font.kMinWidthY / 2)
                poly0[3] = rotation_matrix @ Vec2(
                    -kMinWidthT, -self.font.kMinWidthY / 2
                )
            elif a1 in [1, 6]:
                ...
                poly0[3] = rotation_matrix @ Vec2(
                    -kMinWidthT, -self.font.kMinWidthY - kMinWidthT
                )
            elif a1 == 22:  # 箱型右上角
            ...
            if a2 == 0:
                ...
                poly0[1] = rotation_matrix @ Vec2(
                    kMinWidthT, self.font.kAdjustKakatoL[kakato_adjustment]
                )
                poly0[2] = rotation_matrix @ Vec2(
                    -kMinWidthT,
                    self.font.kAdjustKakatoL[kakato_adjustment] + kMinWidthT,
                )
            elif a2 == 23:
                poly0[1] = rotation_matrix @ Vec2(
                    kMinWidthT, self.font.kAdjustKakatoR[kakato_adjustment]
                )
                poly0[2] = rotation_matrix @ Vec2(
                    -kMinWidthT,
                    self.font.kAdjustKakatoR[kakato_adjustment] + kMinWidthT,
                )
            elif a2 in [24, 32]:
                if vec_1.x == vec_2.x:
            ...
            path = svgwrite.path.Path(
                d="M" + (" L".join(poly0)), stroke="black", stroke_width=0, fill="black"
            )
            self.canvas.add(path)
            ...
            if a1 in [22, 27]:  # box's right top corner
                ...
                poly += (
                    [Vec2(+kMinWidthT, +kMinWidthT), Vec2(-kMinWidthT, 0)]
                    if vec_1.x == vec_2.x
                    else (
                        [
                            Vec2(+kMinWidthT, +kMinWidthT - 1),
                ...
                poly = [
                    vec_1
                    + rotation_matrix @ Vec2(kMinWidthT, self.font.kMinWidthY * 0.5),
                    vec_1
                    + rotation_matrix
                    @ Vec2(
                        kMinWidthT + kMinWidthT * 0.5,
                        self.font.kMinWidthY * 0.5 + self.font.kMinWidthY,
                    ),
                    vec_1
                    + rotation_matrix
                    @ Vec2(
                        kMinWidthT - 2,
                        self.font.kMinWidthY * 0.5 + self.font.kMinWidthY * 2 + 1,
                    ),
                ]
                if vec_1.x != vec_2.x:
                    poly[2] = Vec2(
                        vec_1.x
                        + (kMinWidthT - 2) * sinrad
                        + (self.font.kMinWidthY * 0.5 + self.font.kMinWidthY * 2)
                        * cosrad,
                        vec_1.y
                        + (kMinWidthT + 1) * -cosrad
                        + (self.font.kMinWidthY * 0.5 + self.font.kMinWidthY * 2)
            ...
            if (
                ...
                and a2 == 1
                or a1 == 6
                and (a2 == 0 or vec_1.x != vec_2.x and a2 == 5)
            ):
                # 鈎の横棒の最後の丸
                ...
                poly = [
                    ...
                    Vec2(
                        vec_2.x
                        - cosrad * kMinWidthT * 0.9
                        + -sinrad
                        * -kMinWidthT
                        * 0.9,  # typo? (- cosrad should be + cosrad)
                        vec_2.y
                        + sinrad * kMinWidthT * 0.9
                        + cosrad * -kMinWidthT * 0.9,
                    ),
                ...
                poly = [str(i) for i in poly]
                path = svgwrite.path.Path(
                    d=" ".join(poly), stroke="black", stroke_width=0, fill="black"
                )
                self.canvas.add(path)
            ...
            if a2 in [1, 0, 5]:
                ...
                if a2 == 5:
                    # 鈎の横棒のハネ
                    hane_length = self.font.kWidth * (
                        4 * (1 - opt1 / self.font.kAdjustMageStep) + 1
                    )
                    rv = 1 if vec_1.x < vec_2.x else -1
                    poly = [
            ...
            # 浅い角度
            cosrad, sinrad = (
                Vec2(1, 0) if vec_1.y == vec_2.y else normalize(vec_2 - vec_1)
            )

            rotation_matrix = np.array([[cosrad, -sinrad], [sinrad, cosrad]])
            ...
            poly = [str(i) for i in poly]
            path = svgwrite.path.Path(
                d="M" + (" L".join(poly)), stroke="black", stroke_width=0, fill="black"
            )
            self.canvas.add(path)
            ...
            if a2 == 0:  # 鱗
                uroko_scale = (
                    self.font.kMinWidthU / self.font.kMinWidthY - 1.0
                ) / 4.0 + 1
                poly2 = [
                    vec_2 + rotation_matrix @ Vec2(0, -self.font.kMinWidthY),
                    vec_2
                    + rotation_matrix
                    @ Vec2(-self.font.kAdjustUrokoX[uroko_adjustment] * uroko_scale, 0),
                    vec_2
                    - (rotation_matrix[:, 0] + rotation_matrix[:, 1])
                    * uroko_scale
                    * Vec2(0.5, 1)
                    * Vec2(
                        self.font.kAdjustUrokoX[uroko_adjustment],
                        self.font.kAdjustUrokoY[uroko_adjustment],
                    ),
                ]
