...
class Serif(Font):
    ...
    def adjust_hane(self):
        vert_segments = [
            ...
            if stroke.stroke.a1_100 == 1
            and stroke.stroke.a1_opt == 0
            and stroke.stroke.vec_1.x == stroke.stroke.vec_2.x
        ]

        for serif_stroke in self.serif_strokes:
            stroke = serif_stroke.stroke
            if (
                (stroke.a1_100 == 1 or stroke.a1_100 == 2 or stroke.a1_100 == 6)
                and stroke.a1_opt == 0
                and stroke.a3_100 == 4
                and stroke.a3_opt == 0
            ):
    ...
    def adjust_mage(self):
        ...
        for serif_stroke in self.serif_strokes:
            ...
            if (
                stroke.a1_100 == 1
                and stroke.a1_opt == 0
                and stroke.vec_1.y == stroke.vec_2.y
            ):
                ...
                stroke.a1_100 == 3
                and stroke.a1_opt == 0
                and stroke.vec_2.y == stroke.vec_3.y
            ):
                hori_segments.append(
                    Namespace(
                        **{
            ...
            if is_target:
                for another_hori_segment in hori_segments:
                    ...
                    if (
                        ...
                        and np.round(np.abs(y - other_y), 5)
                        < self.kMinWidthT * self.kAdjustMageStep
                    ):
                        serif_stroke.mage_adjustment += self.kAdjustMageStep - np.floor(
                            np.abs(y - other_y) / self.kMinWidthT
                        )
    ...
    def adjust_tate(self):
        vert_segments = [
            ...
            for stroke in self.serif_strokes
            if (
                stroke.stroke.a1_100 == 1
                or stroke.stroke.a1_100 == 3
                or stroke.stroke.a1_100 == 7
            )
            and stroke.stroke.a1_opt == 0
            and stroke.stroke.vec_1.x == stroke.stroke.vec_2.x
        ]
        ...
        for vert_segment in vert_segments:
            ...
            for another_vert_segment in vert_segments:
                ...
                if (
                    ...
                    and np.round(np.abs(x - other_x), 5)
                    < self.kMinWidthT * self.kAdjustTateStep
                ):
                    serif_stroke.tate_adjustment += self.kAdjustTateStep - np.floor(
                        np.abs(x - other_x) / self.kMinWidthT
                    )
                    if (
                        serif_stroke.tate_adjustment > self.kAdjustTateStep
                        or serif_stroke.tate_adjustment == self.kAdjustTateStep
                        and (stroke.a2_opt_1 != 0 or stroke.a2_100 != 0)
                    ):
                        serif_stroke.tate_adjustment = self.kAdjustTateStep
    ...
    def adjust_kakato(self):
        def loop1(serif_stroke: SerifStroke):
            ...
            if (
                stroke.a1_100 == 1
                and stroke.a1_opt == 0
                and (stroke.a3_100 == 13 or stroke.a3_100 == 23)
                and stroke.a3_opt == 0
            ):
                ...
                def loop2(k: int):
                    if (
                        any(
                            [
                                stroke != serif_stroke.stroke
                                and serif_stroke.stroke.is_cross_box(
                                    Vec2(
                                        stroke.vec_2.x - self.kAdjustKakatoRangeX / 2,
                                        stroke.vec_2.y + self.kAdjustKakatoRangeY[k],
                                    ),
                                    Vec2(
                                        stroke.vec_2.x + self.kAdjustKakatoRangeX / 2,
                                        stroke.vec_2.y
                                        + self.kAdjustKakatoRangeY[k + 1],
                                    ),
                                )
                                for serif_stroke in self.serif_strokes
                            ]
                        )
                        or np.round(stroke.vec_2.y + self.kAdjustKakatoRangeY[k + 1], 5)
                        > 200
                        or np.round(stroke.vec_2.y - stroke.vec_1.y)
                        < self.kAdjustKakatoRangeY[k + 1]
                    ):  # for thin box
                ...
                def loop4(k: int):
                    a = (
                        ...
                        if stroke.vec_1.y == stroke.vec_2.y
                            ...
                            normalize(
                                Vec2(
                                    stroke.vec_1.x - stroke.vec_2.x,
                                    stroke.vec_1.y - stroke.vec_2.y,
                                )
                            )
                            if stroke.vec_2.x - stroke.vec_1.x < 0
                            else normalize(
                                Vec2(
                                    stroke.vec_2.x - stroke.vec_1.x,
                                    stroke.vec_2.y - stroke.vec_1.y,
                                )
                            )
                        )
                    )
                    ...
                    tx = (
                        stroke.vec_2.x
                        - self.kAdjustUrokoLine[k] * cosrad
                        - 0.5 * sinrad
                    )  # typo? (sinrad should be -sinrad ?)
                    ty = (
                        stroke.vec_2.y
                        - self.kAdjustUrokoLine[k] * sinrad
                        - 0.5 * cosrad
                    )
                    tlen = (
                        stroke.vec_2.x - stroke.vec_1.x
                    ...
                    if np.round(tlen, 5) < self.kAdjustUrokoLength[k] or any(
                        [
                            stroke != serif_stroke.stroke
                            and serif_stroke.stroke.is_cross(Vec2(tx, ty), stroke.vec_2)
                            for serif_stroke in self.serif_strokes
                        ]
                    ):
                ...
                hori_segments.append(
                    Namespace(
                        **{
                            "stroke": stroke,
                            "serif_stroke": serif_stroke,
                            "is_target": stroke.a3_100 == 0
                            and stroke.a3_opt == 0
                            and serif_stroke.uroko_adjustment == 0,
                            "y": stroke.vec_1.y,
                            "x1": stroke.vec_1.x,
                            "x2": stroke.vec_2.x,
                        }
            ...
            if is_target:
                ...
                for another_hori_segment in hori_segments:
                    ...
                    if (
                        ...
                        pressure += np.power(
                            (self.kAdjustUroko2Length - np.abs(y - other_y)), 1.1
                        )
                serif_stroke.uroko_adjustment = int(
                    np.min(
                        [
                            np.floor(pressure / self.kAdjustUroko2Length),
                            self.kAdjustUroko2Step,
                        ]
                    )
                )
    ...
    def adjust_kirikuchi(self):
        ...
        def loop5(serif_stroke: SerifStroke):
            ...
            if (
                stroke.a1_100 == 2
                and stroke.a1_opt == 0
                and stroke.a2_100 == 32
                and stroke.a2_opt == 0
                and stroke.vec_1.x > stroke.vec_2.x
                and stroke.vec_1.y < stroke.vec_2.y
                and any(
                    [
                        hori_segment.x1 < stroke.vec_1.x
                        and hori_segment.x2 > stroke.vec_1.x
                        and hori_segment.y == stroke.vec_1.y
                        for hori_segment in hori_segments
                    ]
                )
            ):
        ...
        for serif_stroke in self.serif_strokes:
            ...
            if stroke.a1_100 == 0:  # TODO: Transforms
                ...
                if stroke.a3_100 == 4:
                    m = (
                        Vec2(0, self.kMage)
                        if all(stroke.vec_1 == stroke.vec_2)
                ...
                if stroke.a3_100 == 4:
                    vec_d = (
                        Vec2(0, -self.kMage)
                        if stroke.vec_2.x == stroke.vec_3.x
                        else (
                            Vec2(-self.kMage, 0)
                            if stroke.vec_2.y == stroke.vec_3.y
                    ...
                    stroke_drawer.draw_curve(
                        ...
                        stroke.vec_2,
                        vec_t1,
                        stroke.a2_100 + serif_stroke.kirikuchi_adjustment * 100,
                        0,
                        stroke.a2_opt_2,
                        0,
                        stroke.a2_opt_3,
                    ...
                    stroke_drawer.draw_curve(
                        vec_t1,
                        stroke.vec_3,
                        stroke.vec_3 - Vec2(self.kMage, 0),
                        2,
                        14,
                        stroke.a2_opt_2,
                        serif_stroke.hane_adjustment,
                ...
                vec_d2 = (
                    Vec2(0, -self.kMage)
                    if all(stroke.vec_2 == stroke.vec_3)
                ...
                if not (
                    stroke.a3_100 == 5
                    and stroke.a3_opt_1 == 0
                    and not (
                        (
                            stroke.vec_2.x < stroke.vec_3.x
                            and stroke.vec_3.x - vec_t2.x > 0
                        )
                        or (
                            stroke.vec_2.x > stroke.vec_3.x
                            and vec_t2.x - stroke.vec_3.x > 0
                        )
                    )
                ):
                    opt2 = (
                        0
                        if (stroke.a3_100 == 5 and stroke.a3_opt_1 == 0)
                ...
                stroke_drawer.draw_curve(
                    ...
                    stroke.vec_2,
                    stroke.vec_3,
                    stroke.a2_100 + stroke.a2_opt_1 * 100,
                    1,
                    stroke.a2_opt_2,
                    0,
                    stroke.a2_opt_3,
                ...
                if stroke.a3_100 == 4:
                    vec_d = (
                        Vec2(0, -self.kMage)
                        if stroke.vec_3.x == stroke.vec_4.x
                        else (
                            Vec2(-self.kMage, 0)
                            if stroke.vec_3.y == stroke.vec_4.y
                    ...
                    stroke_drawer.draw_curve(
                        vec_t1,
                        stroke.vec_4,
                        stroke.vec_4 - Vec2(self.kMage, 0),
                        1,
                        14,
                        0,
                        serif_stroke.hane_adjustment,
