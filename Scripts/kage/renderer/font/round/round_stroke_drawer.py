...
class RoundStrokeDrawer:
        ...
        if not append_last:
            self.canvas.add(
                svgwrite.path.Path(
                    ...
                    stroke="black",
                    stroke_width=self.font.kWidth * 2,
                    fill="none",
            ...
            self.canvas.elements[-1].push(
                generate_d(
                    vec_1, vec_s1, vec_s2, vec_2, is_quadratic, append_last, is_smooth
                )
            )
    ...
    def draw_bezier(
        ...
        RoundStrokeDrawer.__draw_curve_universal(
            self, vec_1, vec_s1, vec_s2, vec_2, a1, a2, False, is_smooth, append_last
        )
    ...
    def draw_curve(
        ...
        RoundStrokeDrawer.__draw_curve_universal(
            self, vec_1, vec_s, vec_s, vec_2, a1, a2, True, is_smooth, append_last
        )

    def draw_line(
        self, vec_1: Vec2, vec_2: Vec2, a1: int, a2: int, append_last: bool = False
    ):
        if not append_last:
            append_last = if_in_merge_range(vec_1, self.last_point, self.font.kWidth)
        ...
        if not append_last:
            self.canvas.add(
                svgwrite.path.Path(
                    d=f"M{vec_1.x},{vec_1.y} L{vec_2.x},{vec_2.y}",
                    stroke="black",
                    stroke_width=self.font.kWidth * 2,
                    fill="none",
                    stroke_linejoin="round",
