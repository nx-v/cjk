...
def ternary_search_max(
    f: Callable[[float], float], left: float, right: float, absolute_precision=1e-5
) -> float:
    """
...
def ternary_search_min(
    ...
    return ternary_search_max(lambda x: -f(x), left, right, absolute_precision)
...
def cubic_bezier(
    vec_1: Vec2, vec_s1: Vec2, vec_s2: Vec2, vec_2: Vec2, t: float
) -> Vec2:
    s = 1 - t
...
def cubic_bezier_deriv(
    ...
    return (
        ...
        * (
            t
            * (
                t * (-vec_1 + 3 * vec_s1 - 3 * vec_s2 + vec_2)
                + 2 * (vec_1 - 2 * vec_s1 + vec_s2)
            )
            - vec_1
            + vec_s1
        )
...
def divide_curve(
    dot_1: Vec2, dot_s1: Vec2, dot_2: Vec2, curve: List[Vec2]
...
def find_offcurve(curve: List[Vec2], dot_s: Vec2) -> Tuple[Vec2, Vec2, Vec2]:
    ...
    minx = ternary_search_min(
        lambda tx: sum(
            [
                (p[0] - quadratic_bezier(dot_n1.x, tx, dot_n2.x, i / (len(curve) - 1)))
                ** 2
    ...
    miny = ternary_search_min(
        lambda ty: sum(
            [
                (p[0] - quadratic_bezier(dot_n1.y, ty, dot_n2.y, i / (len(curve) - 1)))
                ** 2
                for i, p in enumerate(curve)
            ]
        ),
...
def generate_flatten_curve(
    ...
    vec_2: Vec2,
    k_rate: float,
    width_func: Callable[[float], float],
    ...
    for tt in range(0, 1001, k_rate):
        ...
        i_dot = (
            Vec2(-width, 0)
            if all(i_dot == Vec2(0, 0))
            else normalize(Vec2(-i_dot.y, i_dot.x), width)
        )  # XXX ???
