...
def export_font_svgs(fontpath: str, out_dir: str, overwrite: bool = False, manifest: dict = None) -> None:
...
def build_all(in_dir: str, out_dir: str, export_svgs: bool = True, overwrite: bool = False, manifest_name: str = 'manifest.json') -> None:
  ...
  for fontpath in fonts:
    ...
    print(f'Exporting glyphs from {os.path.basename(fontpath)} into {font_out_dir} (overwrite={overwrite})')
    if export_svgs:
      export_font_svgs(fontpath, font_out_dir, overwrite=overwrite, manifest=manifest)
      print('  Done')
...
def parse_args():
  ...
  p.add_argument('--no-svgs', dest='no_svgs', action='store_true', help='Do not export SVG files')
  p.add_argument('--overwrite', dest='overwrite', action='store_true', help='Overwrite existing SVG files')
  p.add_argument('--manifest', dest='manifest', default='manifest.json', help='Write manifest JSON filename in out dir')
  return p.parse_args()

if __name__ == '__main__':
  args = parse_args()
  build_all(args.in_dir, args.out_dir, export_svgs=not args.no_svgs, overwrite=args.overwrite, manifest_name=args.manifest)
